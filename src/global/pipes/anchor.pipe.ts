  ...
  transform(html: string): string {
    if (html) {
      ...
      chain(htmlDoc.getElementsByTagName('a'))
        .filter((anchorLink: HTMLAnchorElement) => anchorLink.getAttribute('href') && anchorLink.getAttribute('href').startsWith('#'))
        .forEach((anchorLink: HTMLAnchorElement) => anchorLink.setAttribute('href', `${this.path}${anchorLink.getAttribute('href')}`))
        .value();
